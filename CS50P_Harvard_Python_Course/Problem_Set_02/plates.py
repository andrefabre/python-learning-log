...
def is_valid(s):
    if not onlyNumAndChar(s):
        return False #if only letters and numbers return true 
    if not stringLength(s):
        return False# if 2 <= len(s) <== 6 return true 
    if not firstTwoCharNotNum(s):
        return False# if first two char are not numbers returns true 
    if not firstCharNotZero(s):
        return False# if first char is not 0 return true 
    if not middleCharNotChar(s):
        return False# if middle chars are not numbers returns true 
    return True
