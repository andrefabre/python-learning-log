...
connector = sqlite3.connect("library.db") # Connects the Python script to the database provided as argument
cursor = connector.cursor() # Used to assign cursor to database
...
def issuer_card():
    Cid = sd.askstring("Issue Card ID", "What is the Issuer\'s Card ID?\t\t\t")
...
def view_record():
    global bk_name, bk_id, bk_status, author_name, clear_and_display
    global tree 
    
    if not tree.focus():
        mb.showerror("Select a row!", "To view a record, you must select it in the table. Please d so before continuing.")
        return
...
def add_record():
    ...
    if surety:
        try:
            ...
            mb.showerror("Book ID already in use!", "The Bood ID you are trying to enter is already in the database, please alter that book\'s record or check any discrepancies on your side")
            
def update_record():
    def update():
        global bk_status, bk_name, bk_id, author_name, clear_and_display
...
def change_availability():
    ...
    if BK_status == "Issued":
        surety = mb.askyesno("Is returned confirmed?", "Has the book been returned to you?")
        if surety:
            cursor.execute("UPDATE Library SET bk_status=?, card_id=? WHERE bk_id=?", ("Available", "N/A", BK_id))
            connector.commit()
        else:
            mb.showinfo("Cannot be returned", "The book satus cannot be set to Available unless it has been returned")
    else:
...
# Initialisig the GUI window and placing all its components

# Variables
lf_bg = "LightSkyBlue" # Left Frame Background Colour
rtf_bg = "DeepSkyBlue" # Right Top Frame Background Colour
rbf_bg = "DodgerBlue" # Right Bottom Frame Background Colout
btn_hlb_bg = "SteelBlue" # Background colour for Head Labels and Buttons
...
Button(RT_frame, text="Change Book Availability", font=btn_font, bg=btn_hlb_bg, width=19, command=delete_inventory).place(x=518, y=30)
